...
args.ash_bound = np.percentile(nmf_components.cpu(), args.ash_bound)
args.react_lower_bound = np.percentile(nmf_components.cpu(), args.react_lower_bound)
args.react_upper_bound = np.percentile(nmf_components.cpu(), args.react_upper_bound)
...
# def eval_datasets(r0, r1, r2, r3, r4, r5, r6, r7, r8, r9, r10, r11, r12, r13, r14, r15, r16, r17, r18, r19, r20, r21, r22, r23, r24, r25, r26, r27, r28, r29, r30, r31, r32, r33, r34, r35, r36, r37, r38, r39, r40, r41, r42, r43, r44, r45, r46, r47, r48, r49, r50, r51, r52, r53, r54, r55, r56, r57, r58, r59, r60, r61, r62, r63, r64, r65, r66, r67, r68, r69, r70, r71, r72, r73, r74, r75, r76, r77, r78, r79, r80, r81, r82, r83, r84, r85, r86, r87, r88, r89, r90, r91, r92, r93, r94, r95, r96, r97, r98, r99, r100, r101, r102, r103, r104, r105, r106, r107, r108, r109, r110, r111, r112, r113, r114, r115, r116, r117, r118, r119, r120, r121, r122, r123, r124, r125, r126, r127, r128, r129, r130, r131, r132, r133, r134, r135, r136, r137, r138, r139, r140, r141, r142, r143, r144, r145, r146, r147, r148, r149, c0, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c12, c13, c14, c15, c16, c17, c18, c19, c20, c21, c22, c23, c24, c25, c26, c27, c28, c29, c30, c31, c32, c33, c34, c35, c36, c37, c38, c39, c40, c41, c42, c43, c44, c45, c46, c47, c48, c49, c50, c51, c52, c53, c54, c55, c56, c57, c58, c59, c60, c61, c62, c63, c64, c65, c66, c67, c68, c69, c70, c71, c72, c73, c74, c75, c76, c77, c78, c79, c80, c81, c82, c83, c84, c85, c86, c87, c88, c89, c90, c91, c92, c93, c94, c95, c96, c97, c98, c99, c100, c101, c102, c103, c104, c105, c106, c107, c108, c109, c110, c111, c112, c113, c114, c115, c116, c117, c118, c119, c120, c121, c122, c123, c124, c125, c126, c127, c128, c129, c130, c131, c132, c133, c134, c135, c136, c137, c138, c139, c140, c141, c142, c143, c144, c145, c146, c147, c148, c149):
    global nmf_components
    global final_fpr, final_avg_fpr, final_auroc, final_avg_auroc
    global id_feats, ood_feats, nmf_id_feats, nmf_ood_feats
    global id_feats_test, ood1_feats, ood2_feats, ood3_feats, ood4_feats
    global nmf_id_feats_test, nmf_ood1_feats, nmf_ood2_feats, nmf_ood3_feats, nmf_ood4_feats
    ...
    with torch.no_grad():
        ...
        m_nmf_id_feats = torch.clone(nmf_id_feats)
        m_nmf_ood_feats = torch.clone(nmf_ood_feats)
        m_nmf_id_feats_test = torch.clone(nmf_id_feats_test)
        m_nmf_ood1_feats = torch.clone(nmf_ood1_feats)
        m_nmf_ood2_feats = torch.clone(nmf_ood2_feats)
        m_nmf_ood3_feats = torch.clone(nmf_ood3_feats)
        m_nmf_ood4_feats = torch.clone(nmf_ood4_feats)
        trans = torch.clone(nmf_components)
        if args.num_component == 15:
        ...
        for (i, j) in zip(range(trans.shape[0]), react_list):
            # trans[i] = react(trans[i], np.percentile(trans[i].cpu(), j))
            trans[i] = react(trans[i], j)

        for (i, j) in zip(range(trans.shape[0]), ash_list):
            # trans[i] = ash_s_ins(trans[i], j)
            trans[i] = ash_s_thre(trans[i], j)

        # for (i, j) in zip(range(50), scale_list):
            # trans[i] = trans[i] * j

        # trans = trans * torch.exp(torch.Tensor([1])).cuda() # checking

        m_id_feats = m_id_feats - m_nmf_id_feats.mm(nmf_components).cuda()
        m_id_feats = m_id_feats + m_nmf_id_feats.mm(trans).cuda()
        # m_id_feats = m_id_feats + m_nmf_id_feats.mm(trans * torch.exp(torch.Tensor([1])).cuda()).cuda()
        m_ood_feats = m_ood_feats - m_nmf_ood_feats.mm(nmf_components).cuda()
        m_ood_feats = m_ood_feats + m_nmf_ood_feats.mm(trans).cuda()
        # m_ood_feats = m_ood_feats + m_nmf_ood_feats.mm(trans * torch.exp(torch.Tensor([1])).cuda()).cuda()
        m_id_feats_test = m_id_feats_test - m_nmf_id_feats_test.mm(nmf_components).cuda()
        m_id_feats_test = m_id_feats_test + m_nmf_id_feats_test.mm(trans).cuda()
        # m_id_feats_test = m_id_feats_test + m_nmf_id_feats_test.mm(trans * torch.exp(torch.Tensor([1])).cuda()).cuda()
        m_ood1_feats = m_ood1_feats - m_nmf_ood1_feats.mm(nmf_components).cuda()
        m_ood1_feats = m_ood1_feats + m_nmf_ood1_feats.mm(trans).cuda()
        # m_ood1_feats = m_ood1_feats + m_nmf_ood1_feats.mm(trans * torch.exp(torch.Tensor([1])).cuda()).cuda()
        m_ood2_feats = m_ood2_feats - m_nmf_ood2_feats.mm(nmf_components).cuda()
        m_ood2_feats = m_ood2_feats + m_nmf_ood2_feats.mm(trans).cuda()
        # m_ood2_feats = m_ood2_feats + m_nmf_ood2_feats.mm(trans * torch.exp(torch.Tensor([1])).cuda()).cuda()
        m_ood3_feats = m_ood3_feats - m_nmf_ood3_feats.mm(nmf_components).cuda()
        m_ood3_feats = m_ood3_feats + m_nmf_ood3_feats.mm(trans).cuda()
        # m_ood3_feats = m_ood3_feats + m_nmf_ood3_feats.mm(trans * torch.exp(torch.Tensor([1])).cuda()).cuda()
        m_ood4_feats = m_ood4_feats - m_nmf_ood4_feats.mm(nmf_components).cuda()
        m_ood4_feats = m_ood4_feats + m_nmf_ood4_feats.mm(trans).cuda()
        # m_ood4_feats = m_ood4_feats + m_nmf_ood4_feats.mm(trans * torch.exp(torch.Tensor([1])).cuda()).cuda()

        # m_id_feats = ash_s_io(m_id_feats, 85)
        # m_ood_feats = ash_s_io(m_ood_feats, 85)
        # m_id_feats_test = ash_s_io(m_id_feats_test, 85)
        # m_ood1_feats = ash_s_io(m_ood1_feats, 85)
        # m_ood2_feats = ash_s_io(m_ood2_feats, 85)
        # m_ood3_feats = ash_s_io(m_ood3_feats, 85)
        # m_ood4_feats = ash_s_io(m_ood4_feats, 85)

        # m_id_feats = ash_s_io(m_id_feats, 90)
        # m_ood_feats = ash_s_io(m_ood_feats, 90)
        # m_id_feats_test = ash_s_io(m_id_feats_test, 90)
        # m_ood1_feats = ash_s_io(m_ood1_feats, 90)
        # m_ood2_feats = ash_s_io(m_ood2_feats, 90)
        # m_ood3_feats = ash_s_io(m_ood3_feats, 90)
        # m_ood4_feats = ash_s_io(m_ood4_feats, 90)

        # m_id_feats = ash_s(m_id_feats, args.percent)
        # m_ood_feats = ash_s(m_ood_feats, args.percent)
        ...
        # m_id_logits = nmf_softmax(m_id_logits)
        # m_ood_logits = nmf_softmax(m_ood_logits)
        # m_id_logits_test = nmf_softmax(m_id_logits_test)
        # m_ood1_logits = nmf_softmax(m_ood1_logits)
        # m_ood2_logits = nmf_softmax(m_ood2_logits)
        # m_ood3_logits = nmf_softmax(m_ood3_logits)
        # m_ood4_logits = nmf_softmax(m_ood4_logits)

        id_scores =  - torch.logsumexp(m_id_logits, axis=1).cpu().detach().numpy()
        ood_scores =  - torch.logsumexp(m_ood_logits, axis=1).cpu().detach().numpy()
    ...
    texture_fpr, texture_auroc, _ = score_get_and_print_results(log, id_scores_test, ood1_scores)
    places365_fpr, places365_auroc, _ = score_get_and_print_results(log, id_scores_test, ood2_scores)
    sun_fpr, sun_auroc, _ = score_get_and_print_results(log, id_scores_test, ood3_scores)
    inaturalist_fpr, inaturalist_auroc, _ = score_get_and_print_results(log, id_scores_test, ood4_scores)
    fpr = fpr * 100
    auroc = auroc * 100
    avg_fpr = (texture_fpr + places365_fpr + sun_fpr + inaturalist_fpr) / 4 * 100
    avg_auroc = (texture_auroc + places365_auroc + sun_auroc + inaturalist_auroc) / 4 * 100
    if final_fpr > fpr:
    # if final_auroc < auroc:
    # if final_avg_fpr > avg_fpr:
        final_fpr = fpr
        final_auroc = auroc
        final_avg_fpr = avg_fpr
        final_avg_auroc = avg_auroc

        final_fpr1 = texture_fpr
        final_fpr2 = places365_fpr
        final_fpr3 = sun_fpr
        final_fpr4 = inaturalist_fpr
        final_auroc1 = texture_auroc
        final_auroc2 = places365_auroc
        final_auroc3 = sun_auroc
        final_auroc4 = inaturalist_auroc
    print("fpr: %.2f" % (final_fpr))
    print("auroc: %.2f" % (final_auroc))
...
# logger = JSONLogger(path="./logging/resnet50_imagenet_seed" + str(args.seed) + ".log")
...
#     print("avg_fpr: %.2f" % ((texture_fpr + places365_fpr + sun_fpr + inaturalist_fpr) / 4 * 100))
#     print("avg_auroc: %.2f" % ((texture_auroc + places365_auroc + sun_auroc + inaturalist_auroc) / 4 * 100))
