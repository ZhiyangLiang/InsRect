...
# joblib.dump(nmf, "../data/nmf_intuitive_test_cifar100.pkl")
# nmf = joblib.load("../data/nmf_intuitive_test_cifar10_num50.pkl")
# nmf = joblib.load("../data/nmf_intuitive_test_cifar100.pkl")
...
def evaluate():
    ...
    global id_error, ood_error, nmf_tran_id_feats, nmf_tran_ood_feats
    
    m_trans = torch.clone(trans)
    ...
    for (i, j) in zip(range(m_trans.shape[0]), (((w_id_feats - w_ood_feats) * s_trans) < np.percentile(((w_id_feats - w_ood_feats) * s_trans).cpu(), 1))):
