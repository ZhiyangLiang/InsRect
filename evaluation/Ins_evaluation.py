...
def get_measures(_pos, _neg, recall_level=recall_level_default):
    ...
    return auroc, aupr, fpr, aupr_in, aupr_out

def print_measures(mylog, auroc, aupr, fpr, aupr_in, aupr_out):
    print('& {:.2f} & {:.2f} & {:.2f} & {:.2f} & {:.2f}'.format(100*fpr, 100*auroc, 100*aupr, 100*aupr_in, 100*aupr_out))
    mylog.debug('& {:.2f} & {:.2f} & {:.2f} & {:.2f} & {:.2f}'.format(100*fpr, 100*auroc, 100*aupr, 100*aupr_in, 100*aupr_out))
...
def get_and_print_results(mylog, ood_loader, in_score):
    model.eval()
    aurocs, auprs, fprs, aupr_ins, aupr_outs = [], [], [], [], []
    ood_score = get_ood_scores(ood_loader)
    measures = get_measures(ood_score, in_score)
    aurocs.append(measures[0]); auprs.append(measures[1]); fprs.append(measures[2]); aupr_ins.append(measures[3]); aupr_outs.append(measures[4])
    auroc = np.mean(aurocs); aupr = np.mean(auprs); fpr = np.mean(fprs); aupr_in = np.mean(aupr_ins); aupr_out = np.mean(aupr_outs)
    print_measures(mylog, auroc, aupr, fpr, aupr_in, aupr_out)
    return fpr, auroc, aupr, aupr_in, aupr_out
...
def evaluate():
    ood_fpr1, ood_auroc1, ood_aupr1, ood_aupr_in1, ood_aupr_out1 = score_get_and_print_results(log, id_score_test, ood_score1)
    ood_fpr2, ood_auroc2, ood_aupr2, ood_aupr_in2, ood_aupr_out2 = score_get_and_print_results(log, id_score_test, ood_score2)
    ood_fpr3, ood_auroc3, ood_aupr3, ood_aupr_in3, ood_aupr_out3 = score_get_and_print_results(log, id_score_test, ood_score3)
    ood_fpr4, ood_auroc4, ood_aupr4, ood_aupr_in4, ood_aupr_out4 = score_get_and_print_results(log, id_score_test, ood_score4)
    ood_fpr5, ood_auroc5, ood_aupr5, ood_aupr_in5, ood_aupr_out5 = score_get_and_print_results(log, id_score_test, ood_score5)
    ood_fpr6, ood_auroc6, ood_aupr6, ood_aupr_in6, ood_aupr_out6 = score_get_and_print_results(log, id_score_test, ood_score6)
    ...
    print("avg_aupr: %.2f" % ((ood_aupr1 + ood_aupr2 + ood_aupr3 + ood_aupr4 + ood_aupr5 + ood_aupr6) / 6 * 100))
