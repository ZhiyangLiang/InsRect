...
def eval_datasets(clamp0_1, clamp1_1, clamp2_1, clamp0_2, clamp1_2, clamp2_2):
    ...
    # plt.bar(range(342 - args.start - args.end), m_id_feats[cnt][args.start:342 - args.end].cpu(), color=['lightsteelblue'], width=args.width)
    # plt.ylim((0.0, 7.5))
    # plt.xticks([])
    # plt.yticks([])
    # if args.pictype == "png":
    #     plt.savefig("./id_before_" + str(cnt) + "_scale.png")
    # elif args.pictype == "svg":
    #     plt.savefig("./id_before_" + str(cnt) + "_scale.svg")
    # plt.clf()

    plt.bar(range(342 - args.start - args.end), m_ood_feats[cnt][args.start:342 - args.end].cpu(), color=['lightgrey'], width=args.width)
    plt.ylim((0.0, 7.5))
    plt.xticks([])
    plt.yticks([])
    if args.pictype == "png":
        plt.savefig("./ood_before_" + str(cnt) + "_scale.png")
    if args.pictype == "svg":
        plt.savefig("./ood_before_" + str(cnt) + "_scale.svg")
    # plt.clf()
    ...
    m_trans[:, 0] = ash_s_thre(m_trans[:, 0], 1.252)
    m_trans[:, 0] = react(m_trans[:, 0], 1.693)
    m_trans[:, 1] = ash_s_thre(m_trans[:, 1], 1.037)
    m_trans[:, 1] = react(m_trans[:, 1], 1.755)
    m_trans[:, 2] = ash_s_thre(m_trans[:, 2], 1.132)
    m_trans[:, 2] = react(m_trans[:, 2], 1.536)
    ...
    m_id_feats[m_id_feats < 0] = 0
    m_ood_feats[m_ood_feats < 0] = 0


    # plt.bar(range(342 - args.start - args.end), m_id_feats[cnt][args.start:342 - args.end].cpu(), color=['royalblue'], width=args.width)
    # plt.ylim((0.0, 7.5))
    # plt.xticks([])
    # plt.yticks([])
    # if args.pictype == "png":
    #     plt.savefig("./id_after_" + str(cnt) + "_scale.png")
    # elif args.pictype == "svg":
    #     plt.savefig("./id_after_" + str(cnt) + "_scale.svg")
    # plt.clf()
    plt.bar(range(342 - args.start - args.end), m_ood_feats[cnt][args.start:342 - args.end].cpu(), color=['dimgrey'], width=args.width)
    plt.ylim((0.0, 7.5))
    plt.xticks([])
    plt.yticks([])
    if args.pictype == "png":
        plt.savefig("./ood_after_" + str(cnt) + "_scale.png")
    elif args.pictype == "svg":
        plt.savefig("./ood_after_" + str(cnt) + "_scale.svg")
    plt.clf()
    cnt = cnt + 1
