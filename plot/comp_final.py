...
def my_plt_comp(comp_num):
    ...
    if comp_num == 0:
        plt.bar(range(342 - args.start - args.end), nmf_comp[args.start:342 - args.end], color=['paleturquoise'], width=args.width)
    if comp_num == 1:
        plt.bar(range(342 - args.start - args.end), nmf_comp[args.start:342 - args.end], color=['lightgreen'], width=args.width)
    if comp_num == 2:
        plt.bar(range(342 - args.start - args.end), nmf_comp[args.start:342 - args.end], color=['lightcoral'], width=args.width)
    plt.ylim((0.0, 7.5))
    plt.xticks([])
    plt.yticks([])

    if comp_num == 0:
        nmf_comp[nmf_comp < 1.252] = 0
        nmf_comp[nmf_comp > 1.693] = 1.693
        plt.bar(range(342 - args.start - args.end), nmf_comp[args.start:342 - args.end], color=['darkturquoise'], width=args.width)
    if comp_num == 1:
        nmf_comp[nmf_comp < 1.037] = 0
        nmf_comp[nmf_comp > 1.755] = 1.755
        plt.bar(range(342 - args.start - args.end), nmf_comp[args.start:342 - args.end], color=['forestgreen'], width=args.width)
    if comp_num == 2:
        nmf_comp[nmf_comp < 1.132] = 0
        nmf_comp[nmf_comp > 1.536] = 1.536
        plt.bar(range(342 - args.start - args.end), nmf_comp[args.start:342 - args.end], color=['firebrick'], width=args.width)
    plt.ylim((0.0, 7.5))
    plt.xticks([])
    plt.yticks([])
    if comp_num == 0:
        # plt.savefig("./component0_darkturquoise_new.png")
        plt.savefig("./component0_darkturquoise_new.svg")
    if comp_num == 1:
        # plt.savefig("./component1_forestgreen_new.png")
        plt.savefig("./component1_forestgreen_new.svg")
    if comp_num == 2:
        # plt.savefig("./component2_firebrick_new.png")
        plt.savefig("./component2_firebrick_new.svg")
    plt.clf()
